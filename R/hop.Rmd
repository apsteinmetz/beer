...
Oils are where the aroma components reside.  We might imagine but here we can see that more alpha also correlates to more oils.
```{r}
# Now oils
...
hop_type <- hops %>%
  ungroup() %>%
  select(-variety) %>%
  mutate(cluster = as.character(kmeans(cbind(beta,total_oil),centers=3)$cluster))


# plot alpha vs oil by bittering type
hop_type %>%
  ggplot(aes(beta,total_oil,color=cluster,shape=type)) +
  geom_point(size=3)
```
