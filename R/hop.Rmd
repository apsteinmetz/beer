...
The format of this note is a data science notebook.  The code to replicate the work and present all the data is presented along with the descriptive text.  In the web page version there is a button at at the top labeled "code" which you can use to toggle code visibility on or off.  **I recommend you turn code visibility "off" to begin with.  This is more readable that way.**
...
Oils are where the aroma components reside.  We might suspect that more oil is associated with aroma hops but here we can see that bittering hops also have more more oils.
```{r}
# Now oils
...
More interesting, perhaps are the choices of aroma descriptors by hop type.  "Noble" is never used to describe bittering hops, as we would expect.  "Floral" is more popular for aroma hops.  "Spicy" is used more for bittering hops.
...
set.seed(1)
model <- randomForest(type~.,data=hops_full[,2:142],
                      localImp = TRUE,
#                      replace = FALSE,
#                     maxnodes = 12,
                      importance = TRUE)
...
# Conclusion
This has been a practice exercise just to demonstrate some of the tools we can use to explore relationships in brewing data.  I look forward to delving in to larger data sets that have more mysteries to uncover.
